...
import com.rsfriend.client.HardReference;
import com.rsfriend.client.SoftReference;
...
public class RSItem extends MethodProvider {
	...
	public RSItem(final MethodContext ctx, final com.rsfriend.client.RSItem item) {
		super(ctx);
		id = item.getID();
	...
	public RSItemDef getDefinition() {
		try {
			com.rsfriend.client.Node ref = methods.nodes.lookup(methods.client.getRSItemDefLoader(), id);

			if (ref != null) {
				if (ref instanceof HardReference) {
					return new RSItemDef((com.rsfriend.client.RSItemDef) (((HardReference) ref).get()));
				} else if (ref instanceof SoftReference) {
					Object def = ((SoftReference) ref).getReference().get();

					if (def != null) {
						return new RSItemDef((com.rsfriend.client.RSItemDef) def);
					}
				}
			}
			return null;
