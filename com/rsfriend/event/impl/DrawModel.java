...
import com.rsfriend.script.wrappers.RSModel;
...
public class DrawModel implements PaintListener, MouseListener {
	...
	public void onRepaint(Graphics render) {
		drawRect(render);
		if (enabled[0]) {
			for (com.rsfriend.script.wrappers.RSObject o : ctx.objects.getAll()) {
		...
		if (enabled[1]) {
			for (com.rsfriend.script.wrappers.RSCharacter c : ctx.players.getAll()) {
		...
		if (enabled[2]) {
			for (com.rsfriend.script.wrappers.RSCharacter c : ctx.npcs.getAll()) {
				RSModel model = c.getModel();
				if (model != null) {
